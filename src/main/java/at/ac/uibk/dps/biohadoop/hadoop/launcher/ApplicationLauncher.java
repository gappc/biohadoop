	...
	private static Callable<ApplicationId> generateCallable(
		...
		return new Callable<ApplicationId>() {
			...
			public ApplicationId call() throws Exception {
				...
				Object parameter = applicationConfig.getAlgorithmConfiguration()
						.buildParameters();
				Algorithm<?, ?> algorithm = applicationConfig.getAlgorithm().newInstance();
				((Algorithm<?, Object>)algorithm).compute(applicationId, parameter);
