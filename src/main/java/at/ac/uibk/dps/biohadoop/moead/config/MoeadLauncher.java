...
public class MoeadLauncher implements Launcher {

	private static Logger LOGGER = LoggerFactory.getLogger(MoeadLauncher.class);
	...
	public boolean isConfigurationValid(String configFilename) {
		...
		String outputFilename = moeadConfig.getAlgorithmConfig()
				.getOutputFile();
	...
	private void launchAlgorithm(MoeadConfig config) throws Exception {
		final MoeadAlgorithmConfig ac = ((MoeadConfig) config)
				.getAlgorithmConfig();

		Application application = new Application("MOEAD-"
	...
	private void launchMasterEndpoints(MoeadConfig config) throws Exception {
		for (String masterEndpoint : ((MoeadConfig) config)
				.getMasterEndpoints()) {
			Class.forName(masterEndpoint).newInstance();
		}
	}

	private void launchWorkers(MoeadConfig config, String configFilename)
			throws Exception {
	...
	private List<String> getWorkerList(MoeadConfig config) {
